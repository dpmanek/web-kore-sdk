(function (factory) {
	...
	return function koreBotChat() {
		...
		var returnFun = (function ($, KRPerfectScrollbar) {
			...
			audioMsgs = [];
			speechSyn = null;
			audioPlaying = false; // Audio context
				...
				if ('boolean' === typeof navigator['onLine']) {
					if (navigator.onLine) {
						...
						if (
							bot &&
							bot.RtmClient &&
							window.KoreSDK?.chatConfig?.syncMessages?.onNetworkResume?.enable
						) {
							bot.getHistory({
								forHistorySync: true,
								limit:
									window.KoreSDK?.chatConfig?.syncMessages?.onNetworkResume
										?.batchSize,
							});
						}
					} else {
			...
			window.onresize = function (event) {
				...
				for (var i = 0; i < quickReplyDivs.length; i++) {
					...
					if (btnsParentDiv[0].hasChildNodes()) {
						if (
							leftScrollBtn &&
							leftScrollBtn.length &&
							leftScrollBtn[0] &&
							leftScrollBtn[0].classList
						) {
						...
						if (
							rightScrollBtn &&
							rightScrollBtn.length &&
							rightScrollBtn[0] &&
							rightScrollBtn[0].classList
						) {
			...
			chatWindow.prototype.init = function () {
				...
				if (me.config && me.config.maxReconnectionAPIAttempts) {
					me.config.botOptions.maxReconnectionAPIAttempts =
						me.config.maxReconnectionAPIAttempts;
				}
			...
			chatWindow.prototype.bindEvents = function () {
				...
				_chatContainer
					.off('click', '.minimized')
					.on('click', '.minimized,.minimized-title', function (event) {
						if (me.config.multiPageApp && me.config.multiPageApp.enable) {
							me.setLocalStoreItem('kr-cw-state', 'open');
						}
						_chatContainer.removeClass('minimize');
			...
			chatWindow.prototype.bindSDKEvents = function () {
				...
				me.bot.on('reconnected', (response) => {
					if (
						me.config?.syncMessages?.onReconnect?.enable &&
						response?.reconnected
					) {
						me.bot.getHistory({
							forHistorySync: true,
							limit: me.config?.syncMessages?.onReconnect?.batchSize,
						});
					}
				});
			};
			...
			chatWindow.prototype.sendMessage = function (
				...
				if (
					...
					messageToBot['message'] = {
						body: chatInput.text().trim().replace(/\s/g, ' '),
					};
				}
				...
				if (
					$(
						'.kore-chat-window .chat-container [data-kr-msg-id="' +
							msgData.message[0].clientMessageId +
							'"]'
					).length < 1
				) {
					if (
						...
						me.sendMessageViaWebHook(
							...
							function (err) {
								setTimeout(function () {
									var failedMsgEle = $(
										'.kore-chat-window [data-kr-msg-id="' +
											clientMessageId +
											'"]'
									);
									failedMsgEle
			...
			chatWindow.prototype.renderMessage = function (msgData) {
						...
						if (chatContainerConfig && chatContainerConfig.pickerMainConfig) {
							...
							if (
								...
								if (msgData.message[0].component.payload.delimiter) {
									pickerConfig[1].dateRangeConfig.delimiter =
										msgData.message[0].component.payload.delimiter;
								}
								// $('.typingIndicatorContent').css('display', 'block');
				...
				if (
					...
					if (
						$(
							'.kore-chat-window .chat-container [data-kr-msg-id="' +
								(msgData.messageId || msgData.message[0].clientMessageId) +
								'"]'
						).length < 1 ||
						msgData.renderType === 'inline'
					) {
				...
				if (
					...
					if (
						...
						msgData.message[0] &&
						msgData.message[0].component &&
						typeof msgData.message[0].component != 'object'
					) {
						// agent transfer waiting message speaking
    ...
    <div class="chat-box-controls"> \
           ...
           {{/if}}\
			...
			function onComponentReady(_this, data) {
				...
				_cmpt.append(
					'<button class="removeAttachment" aria-label="Remove attachment" title="Remove attachment">X</button>'
				);
